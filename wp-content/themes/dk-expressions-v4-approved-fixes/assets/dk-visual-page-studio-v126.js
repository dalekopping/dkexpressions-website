(function ($) {
	...
	var saveButton = root.querySelector('[data-dkx-save]');
	...
	function showSelection(data) {
		...
		if (data.elementType === 'media') {
			...
			selectionHelp.textContent = data.fieldKey ? 'Type directly on the canvas. This is connected to the page’s WordPress content field.' : 'Type directly on the canvas. The exact visual position and DK styling remain locked.';
		...
		if (data.elementType === 'text') {
			if (data.fieldKey) {
				...
				overrideMap[data.path] = {
					type: 'text',
					path: data.path,
					originalValue: currentSelection && typeof currentSelection.current === 'string' ? currentSelection.current : '',
					value: data.value || ''
				};
			}
	...
	root.addEventListener('click', function (event) {
		...
		if (viewportButton) {
			root.querySelectorAll('[data-dkx-viewport]').forEach(function (button) {
				button.classList.toggle('is-active', button === viewportButton);
			});
			shell.className = 'dkx-vps__frame-shell is-' + viewportButton.getAttribute('data-dkx-viewport');
			return;
		}

...
<div id="dkx-consent" role="dialog" aria-label="Analytics preferences"><p>We use essential storage to run the site and optional analytics to understand what visitors use.</p><div><button type="button" data-choice="granted">Accept analytics</button><button type="button" data-choice="denied">Essential only</button></div></div>
<script id="dkx-conversion-events">(function(){'use strict';var dl=window.dataLayer=window.dataLayer||[];function push(name,extra){var p=extra||{};p.event=name;dl.push(p);}document.addEventListener('click',function(e){var a=e.target&&e.target.closest?e.target.closest('a'):null;if(!a)return;var href=(a.getAttribute('href')||'').trim(),text=(a.textContent||'').trim();if(/wa\.me|whatsapp/i.test(href))push('whatsapp_click',{link_url:href,link_text:text});if(/\.pdf(?:$|[?#])/i.test(href)||/rate card/i.test(text))push('rate_card_download',{link_url:href,link_text:text});if(/start a project/i.test(text)||/\/contact\/?(?:$|[?#])/i.test(href))push('start_project_click',{link_url:href,link_text:text});},true);document.addEventListener('submit',function(e){var f=e.target;if(!f||f.tagName!=='FORM')return;var marker=(f.getAttribute('action')||'')+' '+(f.id||'')+' '+(f.className||'');if(/contact|project|brief|wpforms|dk-native-entry/i.test(marker))push('form_submit',{form_id:f.id||''});},true);document.addEventListener('DOMContentLoaded',function(){var b=document.getElementById('dkx-consent');if(!b)return;var saved='';try{saved=localStorage.getItem('dkx_consent')||'';}catch(e){}if(!saved)b.style.display='flex';b.addEventListener('click',function(e){var btn=e.target.closest('button[data-choice]');if(!btn)return;window.dkxSetConsent(btn.getAttribute('data-choice'));b.style.display='none';});});})();</script>
<?php;},99);

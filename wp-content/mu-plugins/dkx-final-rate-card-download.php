...
final class DKX_Final_Rate_Card_PDF {
    ...
    private function text( &$s, $x,$y,$text,$size=10,$bold=false,$hex='#FFFFFF' ) {
        $font = $bold ? '/F2' : '/F1';
    ...
    private function para( &$s,$x,$y,$text,$size=9,$hex='#9DB2C2',$bold=false,$chars=86,$lead=12 ) {
        foreach ( $this->wrap($text,$chars) as $line ) {
            $this->text($s,$x,$y,$line,$size,$bold,$hex);
            $y -= $lead;
        }
        return $y;
    }
    private function footer( &$s,$page ) {
        $this->rect($s,0,0,$this->w,31.2,'#07131C');
    ...
    private function topbar( &$s,$label,$accent ) {
        $this->rect($s,0,0,$this->w,$this->h,'#02070C');
        $this->rect($s,0,790.9,$this->w,51,'#07131C');
    ...
    private function card( &$s,$x,$top,$w,$h,$name,$price,$suffix,$desc,$features,$accent,$badge='' ) {
        $this->rect($s,$x,$top-$h,$w,$h,'#07131C');
        $this->rect($s,$x,$top-6,$w,6,$accent);
        $this->text($s,$x+14,$top-28,strtoupper($name),9,true,'#FFFFFF');
        if($badge){
            $this->rect($s,$x+$w-70,$top-36,62,17,$accent);
            $this->text($s,$x+$w-64,$top-31,strtoupper($badge),6,true,'#02070C');
        }
        ...
        foreach($features as $f){
            $y = $this->para($s,$x+14,$y,'- '.$f,7,'#E6EEF3',false,31,9);
        }
    }

    public function render() {
        $INK='#02070C'; $PANEL='#07131C'; $PANEL2='#0A1D2A';
        $BLUE='#40B8FF'; $AQUA='#20D7C8'; $GOLD='#FFC34F'; $PURPLE='#976DFF'; $RED='#FF5364'; $ORANGE='#FF8A4C'; $MUTED='#9DB2C2'; $LIGHT='#E6EEF3';
        ...
        $this->text($s,51,686,'DK',52,true,$BLUE);
        $this->text($s,51,638,'EXPRESSIONS',31,true,'#FFFFFF');
        $this->text($s,51,573,'FINAL RATE CARD',19,true,$GOLD);
        $this->text($s,51,544,'2026 COMMERCIAL EDITION',10,true,$AQUA);
        $this->text($s,51,454,'CLEAR PACKAGES. PREMIUM OUTPUT.',17,true,'#FFFFFF');
        $this->text($s,51,423,'NO HOURLY SURPRISES.',17,true,$BLUE);
        $this->text($s,51,363,'Johannesburg + Cape Town, South Africa',9,false,$MUTED);
        $this->text($s,51,340,'South African Rand  |  Rates exclude VAT',9,false,$MUTED);
        $this->text($s,51,77,'PREMIUM CULTURE  /  CONTENT  /  BRAND STORYTELLING',9,true,'#FFFFFF');
        $this->footer($s,1); $this->add_page($s);

        $s=''; $this->topbar($s,'COMMERCIAL SYSTEM / PRICING AT A GLANCE',$BLUE);
        $this->text($s,45,740,'SELL THE OUTCOME,',25,true,'#FFFFFF');
        $this->text($s,45,706,'NOT THE HOUR.',25,true,$BLUE);
        ...
        $y=607;
        foreach($rows as $r){
            $this->rect($s,45,$y-65,505,65,$PANEL);
            $this->rect($s,45,$y-65,9,65,$r[5]);
            $this->text($s,62,$y-37,$r[0],7.2,true,'#FFFFFF');
            $this->text($s,283,$y-37,$r[1],8,true,$LIGHT);
            $this->text($s,360,$y-37,$r[2],8,true,$r[5]);
            $this->text($s,436,$y-37,$r[3],8,true,$LIGHT);
            $this->text($s,520,$y-37,$r[4],7.2,true,$MUTED);
            $y-=65;
        }
        $this->rect($s,45,79,505,99,$PANEL);
        $this->text($s,60,153,'COMMERCIAL FLOOR',8,true,$GOLD);
        $this->text($s,60,125,'MINIMUM NEW-CLIENT ENGAGEMENT: R7,500 EXCL. VAT',13,true,'#FFFFFF');
        $this->para($s,60,100,'Standalone items below this threshold should be sold inside a campaign, bundle or existing-client scope.',7.5,$LIGHT,false,105,10);
        ...
        foreach($services as $svc){
            list($page,$label,$accent,$title,$desc,$best,$pkgs)=$svc;
            $s=''; $this->topbar($s,$label,$accent);
            $this->text($s,45,737,$title,24,true,'#FFFFFF');
            $this->para($s,45,700,$desc,8.7,$LIGHT,false,112,11);
            $this->para($s,45,671,$best,7.8,$MUTED,true,116,10);
            $x=45;
            foreach($pkgs as $p){
                $this->card($s,$x,610,161,354,$p[0],$p[1],$p[2],$p[3],$p[4],$accent,$p[5]);
                $x+=172;
            }
            if($page===3){
                $this->rect($s,45,62,505,60,$PANEL);
                $this->text($s,60,105,'ADD-ONS',7,true,$BLUE);
                $this->text($s,60,82,'Extra shooter R2,500/day  /  Drone R3,500  /  Rush same-hour edits R1,800  /  Branded frame or AR filter: quoted',7.2,true,'#FFFFFF');
            }
            $this->footer($s,$page); $this->add_page($s);
        }

        $s=''; $this->topbar($s,'04 / OWN THE ATTENTION / BLOG & MEDIA PLACEMENTS',$RED);
        $this->text($s,45,737,'OWN THE ATTENTION',24,true,'#FFFFFF');
        $this->para($s,45,700,'Paid editorial placements on DK Expressions and amplification across social channels, monetising the publishing authority and audience built since 2013.',8.7,$LIGHT,false,112,11);
        ...
        $this->text($s,45,170,'SOCIAL AMPLIFICATION - STANDALONE / ADD-ON',11,true,'#FFFFFF');
        $social=array(array('INSTAGRAM','R300','R500','R1,200'),array('FACEBOOK','R1,100','R2,000','R5,000'),array('X (TWITTER)','R500','R850','R2,100'));
        $this->rect($s,45,133,505,26,$PANEL);
        foreach(array('CHANNEL','1 POST','2 POSTS','5 POSTS') as $i=>$h) $this->text($s,51+$i*126,143,$h,7,true,'#FFFFFF');
        $yy=108;
        foreach($social as $row){
            $this->rect($s,45,$yy,505,25,$PANEL2);
            foreach($row as $i=>$v) $this->text($s,51+$i*126,$yy+9,$v,7.5,$i>0,$i>0?'#FFFFFF':$LIGHT);
            $yy-=25;
        }
        $this->footer($s,6); $this->add_page($s);

        $s=''; $this->topbar($s,'FAST-START BUNDLES / COMMERCIAL RULES',$AQUA);
        $this->text($s,45,737,'FAST-START BUNDLES',24,true,'#FFFFFF');
        $bundles=array(
            array('LAUNCH SPARK','R7,500','Compact launch / announcement package and minimum new-client entry point.'),
            array('EVENT STORY','R15,000','Stronger event storytelling entry point for clients not ready for Signature.'),
            array('BRAND MOMENTUM','R14,500','Focused content / campaign bundle for brands testing an ongoing relationship.')
        );
        $x=45;
        foreach($bundles as $b){
            $this->rect($s,$x,541,161,156,$PANEL);
            $this->rect($s,$x,691,161,6,$AQUA);
            $this->text($s,$x+14,663,$b[0],8,true,$AQUA);
            $this->text($s,$x+14,629,$b[1],20,true,'#FFFFFF');
            $this->para($s,$x+14,601,$b[2],7.2,$MUTED,false,31,10);
            $x+=172;
        }
        $this->text($s,45,505,'COMMERCIAL RULES',20,true,'#FFFFFF');
        $rules=array(
            'All rates are starting prices and exclude VAT.',
            '50% deposit before work begins. No deposit, no booking.',
            'All monthly retainers carry a three-month minimum.',
            'Quote the core / anchor tier first. Entry is the fallback, not the opener.',
            'Discount scope, never rate. If budget drops, remove deliverables.',
            'Photography never sells below R5,000; event coverage never below R6,500.',
            'Minimum new-client engagement: R7,500 excl. VAT.',
            'Travel, accommodation, venue fees, paid media spend, talent fees and third-party costs are excluded unless expressly included.',
            'Custom integrated campaigns, Web & AI builds, complex production and long-term partnerships are quoted to scope.',
            'Rates valid to 31 December 2026 unless superseded by a newer DK Expressions commercial rate card.'
        );
        $yy=471;
        foreach($rules as $r){
            $this->rect($s,48,$yy,6,6,$ORANGE);
            $yy=$this->para($s,65,$yy,$r,7.5,$LIGHT,false,112,10)-6;
        }
        $this->rect($s,45,57,505,91,$PANEL);
        $this->text($s,60,122,'CUSTOM / BESPOKE',8,true,$GOLD);
        $this->text($s,60,97,'Need something outside the packages? We quote the connected solution to scope.',12,true,'#FFFFFF');
